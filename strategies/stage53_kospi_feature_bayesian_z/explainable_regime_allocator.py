...
"""KODEX200 전용 feature를 추가한 Bayesian z-score Stage53 자산배분 전략.

이 파일은 Stage51의 GSG universe와 Bayesian ridge 기대수익 추정은 유지하되,
KODEX200 regression에만 fundamental/technical/option feature를 추가한다.
...
아래의 한 방향 흐름만 따라가면 된다.
    ...
    원자료 -> 월별 자산수익률 / 거시 국면점수 / 일별 KRW 수익률
           -> KODEX200 전용 EPS/valuation/credit/K-ratio/option feature
...
리팩터링의 경계
----------------
이 구현은 Stage52의 CD excess return, GSG universe, Bayesian z allocator를 보존하되
KODEX200 기대수익 regression의 feature set만 확장한다. 따라서 Stage50/51의 SLSQP
위험예산 충족 성과를 보존하지 않는다. 앞선
...
class StrategyConfig:
    ...
    def forecast_features_for_asset(self, asset: str) -> tuple[str, ...]:
        if asset == "KODEX200":
            return (*self.forecast_features, *self.kospi_feature_columns)
...
class KospiFeatureRepository:
    """이전 stage가 만든 KODEX200 전용 월별 feature를 읽는다."""
    ...
    def _read_monthly_csv(path: Path) -> pd.DataFrame:
        if not path.exists():
            raise FileNotFoundError(f"Required KOSPI feature file not found: {path}")
        frame = pd.read_csv(path)
        if "target_month" not in frame.columns:
            raise ValueError(f"{path} has no target_month column")
        frame["target_month"] = pd.PeriodIndex(frame["target_month"], freq="M")
        return frame.set_index("target_month").sort_index()

    def build(self, decision_months: pd.PeriodIndex) -> tuple[pd.DataFrame, dict[str, Any]]:
        ...
        audit = {
            "feature_scope": "KODEX200 regression only",
            "features": list(self.config.kospi_feature_columns),
            "source_files": {
...
class BacktestEngine:
    ...
    def _feature_frame(
        ...
        feature_columns = [
            ...
            *self.config.kospi_feature_columns,
        ]
        available_features = [
        ...
        for month in months:
            ...
            row: dict[str, Any] = {
                ...
                **{
                    ...
                    for column in self.config.kospi_feature_columns
                },
                **{
...
def performance_table(
    ...
    for name, view in periods.items():
        rows.append(
            {
                "Strategy": "Stage53_KospiFeatureBayesianZ",
                "Period": name,
...
def save_weight_artifacts(
    ...
    for period, view in analysis_periods(monthly_result, config).items():
        ...
        ax.set_title(f"Stage53 Portfolio Weights - {period}")
...
def individual_asset_performance_table(
    ...
    for period, view in analysis_periods(monthly_result, config).items():
        rows.append(
            {
                "Period": period,
                "Series": "Stage53_KospiFeatureBayesianZ",
                "Type": "strategy_net",
                "Start": str(view.index.min()),
                "End": str(view.index.max()),
                **_performance_summary(view["return"]),
...
def save_pnl_comparison_artifacts(
    ...
    for period, view in analysis_periods(monthly_result, config).items():
        ...
        returns["Stage53"] = view["return"].astype(float)
        for asset in config.assets:
        ...
        ax.set_title(f"Stage53 vs Single-Asset Buy-and-Hold PnL - {period}")
...
def validation_report(
    ...
    return {
        ...
        "declared_heuristics": {
            ...
            "kospi_feature_columns": list(config.kospi_feature_columns),
...
def run_research(
    *,
    ...
    kospi_features, kospi_feature_audit = KospiFeatureRepository(
        paths, config
    ).build(monthly_returns.index)
    regime_scores = regime_scores.join(kospi_features, how="left")
    ...
    report = validation_report(
        ...
        config,
        {
            ...
            "kospi_features": kospi_feature_audit,
        },
    )
    ...
    if save:
        ...
        kospi_features.to_csv(paths.output / "kospi_feature_inputs.csv")
