...
source = (ROOT / "leveraged_daily_overlay_experiment.py").read_text(encoding="utf-8")
source = source.split("\ndaily_levels = load_daily_open_levels()")[0]
exec(compile(source, str(ROOT / "leveraged_daily_overlay_experiment.py"), "exec"), globals())
...
class GuardConfig:
    ...
    def name(self) -> str:
        return (
            f"mx{self.max_exposure:.2f}_tv{self.target_vol:.2f}_ds{abs(self.dd_start):.2f}"
            f"_df{abs(self.dd_floor):.2f}_mn{self.min_exposure:.2f}_rb{self.rebalance_band:.2f}"
        )
...
def simulate_guard(
    ...
    for j in np.flatnonzero(keep):
        month = months[j]
        base = bases[j]
        normal_exposure = min(cfg.max_exposure, cfg.target_vol / max(vols[j], 1e-6))
        ...
        if current_dd >= cfg.dd_start:
            dd_exposure = cfg.max_exposure
        elif current_dd <= cfg.dd_floor:
            dd_exposure = cfg.min_exposure
        else:
            scale = (current_dd - cfg.dd_floor) / (cfg.dd_start - cfg.dd_floor)
            dd_exposure = cfg.min_exposure + (cfg.max_exposure - cfg.min_exposure) * scale
        desired = float(np.clip(min(normal_exposure, dd_exposure), 0.0, cfg.max_exposure))
        ...
        rows.append({
            ...
            "forecast_vol": vols[j], "vix": vix[j], "debt_weight": debt_w,
...
candidates = [
    GuardConfig(mx, tv, -dd_start, -dd_floor, min_exp, rebalance_band=band)
    for mx, tv, dd_start, dd_floor, min_exp, band in itertools.product(
        [1.00, 1.25, 1.50, 1.75, 2.00],
        [0.22, 0.26, 0.30, 0.35, 0.40],
        [0.02, 0.03, 0.04],
        [0.08, 0.09, 0.10],
        [0.25, 0.50, 0.75],
        [0.05, 0.10],
...
for i, candidate in enumerate(candidates, start=1):
    _, monthly = simulate_guard(arrays, candidate, end="2017-12")
...
ranking.to_csv(RESULTS / "daily_guard_calibration.csv", index=False)
...
winner = GuardConfig(**{field: float(winner_row[field]) for field in GuardConfig.__dataclass_fields__})
print("\n=== LOCKED WINNER ===")
print(winner)
winner_daily, winner_monthly = simulate_guard(arrays, winner)
...
for label, start, end in [("calibration", None, "2017-12"), ("locked_test", "2018-01", None), ("full", None, None)]:
    for strategy, bt in [
        ...
        ("GuardedHard", winner_monthly.loc[start:end] if start else winner_monthly.loc[:end] if end else winner_monthly),
    ]:
...
winner_daily.to_csv(RESULTS / "daily_guard_backtest.csv")
winner_monthly.to_csv(RESULTS / "daily_guard_monthly.csv")
comparison.to_csv(RESULTS / "daily_guard_comparison.csv", index=False)
with (RESULTS / "daily_guard_winner.json").open("w", encoding="utf-8") as f:
