...
def apply_technical_inputs(
    ...
    macro_delta = macro - neutral
    # The daily signal is a veto on optimistic macro views, not an alpha
    # source that can rescue an already below-neutral forecast.  This keeps
    # weak technical evidence from manufacturing expected return for a
    # defensive asset while preserving the original downside view.
    filtered_delta = np.where(
        macro_delta > 0.0,
        confidence * macro_delta,
        macro_delta,
    )
    filtered_macro = neutral + filtered_delta
...
def solve_weights(
    ...
    detail: dict[str, Any] = {
        **values,
            "policy": "OneSidedConfidence_StaticLambda",
...
def run_research(save: bool = True) -> dict[str, Any]:
    ...
    paths = {
        ...
        "Stage21_OneSidedConfidence": technical_path,
    }
    ...
    episodes = pd.concat(
        [
            ...
            drawdown_episodes(
                technical_path, returns, "Stage21_OneSidedConfidence"
            ),
        ],
        ignore_index=True,
    )
    ...
    candidate = full.loc["Stage21_OneSidedConfidence"]
    ...
    report: dict[str, Any] = {
        "strategy": "Stage21_OneSidedConfidence",
        ...
        "feature_policy": {
            ...
            "filtered_macro_mu": (
                "neutral_mu + where(delta>0, confidence*delta, delta); "
                "technical evidence may veto optimistic tilts but never "
                "raise below-neutral macro forecasts"
            ),
            "atr_covariance": (
            ...
            "candidate_count": 1,
            "single_change_from_stage20": "one-sided macro confidence filter",
        },
        "unchanged_controls": {
            "downside_risk_aversion_lambda": 1.0,
    ...
    if save:
        ...
        technical_path.to_csv(
            OUTPUT_DIR / "one_sided_confidence_monthly.csv"
        )
