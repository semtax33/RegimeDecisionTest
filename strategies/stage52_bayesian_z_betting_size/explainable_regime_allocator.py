...
"""Bayesian z-score로 betting size를 정하는 Stage52 자산배분 전략.
...
리팩터링의 경계
----------------
이 구현은 Stage51의 데이터와 Bayesian ridge forecast를 최대한 보존하되 배분 규칙만
바꾼다. 따라서 Stage50/51의 SLSQP 위험예산 충족 성과를 보존하지 않는다. 앞선
...
class StrategyConfig:
    ...
    # RETURN DEFINITION: True이면 Bayesian regression과 실현 PnL에 들어가는
    # 자산수익률을 CD(91일) 기준 초과수익률로 바꾼다. False이면 기존처럼
    # 단순 자산수익률을 사용한다.
    use_risk_free_rate: bool = True
...
def performance_table(
    ...
    for name, view in periods.items():
        rows.append(
            {
                "Strategy": "Stage52_BayesianZBettingSize",
                "Period": name,
...
def individual_asset_performance_table(
    ...
    for period, view in analysis_periods(monthly_result, config).items():
        rows.append(
            {
                "Period": period,
                "Series": "Stage52_BayesianZBettingSize",
                "Type": "strategy_net",
                "Start": str(view.index.min()),
                "End": str(view.index.max()),
                **_performance_summary(view["return"]),
...
def save_pnl_comparison_artifacts(
    ...
    for period, view in analysis_periods(monthly_result, config).items():
        ...
        returns["Stage52"] = view["return"].astype(float)
        for asset in config.assets:
        ...
        ax.set_title(f"Stage52 vs Single-Asset Buy-and-Hold PnL - {period}")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--disable-risk-free",
        action="store_true",
        help="Use simple asset returns instead of CD(91d) excess returns.",
    )
    args = parser.parse_args(argv)
    research = run_research(
        save=True,
        refresh_monthly_market_cache=args.refresh_monthly_market_cache,
        use_risk_free_rate=not args.disable_risk_free,
    )
