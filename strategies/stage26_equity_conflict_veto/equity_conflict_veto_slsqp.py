...
def apply_technical_inputs(
    ...
    agreement = macro_direction * technical_direction
    confidence = np.clip(0.5 * (1.0 + agreement), 0.0, 1.0)
    # KODEX200 has the full OHLCV evidence set and is the intended growth
    # sleeve.  Its technical layer is therefore a conflict veto: agreement or
    # no opinion preserves the macro view, while conflict spends confidence.
    # The bond, gold and oil rules remain byte-for-byte equivalent to Stage 20
    # so the established defensive sleeves are not simultaneously released.
    equity_index = ASSETS.index("KODEX200")
    confidence[equity_index] = np.clip(
        1.0 + min(float(agreement[equity_index]), 0.0), 0.0, 1.0
    )
...
def solve_weights(
    ...
    detail: dict[str, Any] = {
        **values,
            "policy": "EquityConflictVeto_StaticLambda",
...
def run_research(save: bool = True) -> dict[str, Any]:
    ...
    paths = {
        ...
        "Stage26_EquityConflictVeto": technical_path,
    }
    ...
    episodes = pd.concat(
        [
            ...
            drawdown_episodes(
                technical_path, returns, "Stage26_EquityConflictVeto"
            ),
        ],
        ignore_index=True,
    )
    ...
    candidate = full.loc["Stage26_EquityConflictVeto"]
    ...
    report: dict[str, Any] = {
        "strategy": "Stage26_EquityConflictVeto",
        ...
        "feature_policy": {
            ...
            "equity_technical_direction": (
                "equal mean(k_score, price_strength, volume_strength)"
            ),
            "other_asset_technical_direction": "k_score",
            "macro_confidence": (
                "KODEX200: clip(1 + min(agreement,0),0,1); "
                "BOND/GLD/USO: (1+agreement)/2 as in Stage20"
            ),
            "filtered_macro_mu": (
            ...
            "single_change_from_stage20": (
                "conflict-only confidence mapping for KODEX200"
            ),
            "research_status": (
                "exploratory attribution follow-up declared after Stage25"
            ),
        },
        "unchanged_controls": {
            "downside_risk_aversion_lambda": 1.0,
    ...
    if save:
        ...
        technical_path.to_csv(
            OUTPUT_DIR / "equity_conflict_veto_monthly.csv"
        )
