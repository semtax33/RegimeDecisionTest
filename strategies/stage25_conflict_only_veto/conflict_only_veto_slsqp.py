...
def apply_technical_inputs(
    ...
    # No technical opinion and agreement both leave the causal macro view
    # intact.  Only a conflicting daily signal spends confidence, in direct
    # proportion to its bounded magnitude.  There is no fitted threshold.
    confidence = np.clip(1.0 + np.minimum(agreement, 0.0), 0.0, 1.0)
...
def solve_weights(
    ...
    detail: dict[str, Any] = {
        **values,
            "policy": "ConflictOnlyVeto_StaticLambda",
...
def run_research(save: bool = True) -> dict[str, Any]:
    ...
    paths = {
        ...
        "Stage25_ConflictOnlyVeto": technical_path,
    }
    ...
    episodes = pd.concat(
        [
            ...
            drawdown_episodes(
                technical_path, returns, "Stage25_ConflictOnlyVeto"
            ),
        ],
        ignore_index=True,
    )
    ...
    candidate = full.loc["Stage25_ConflictOnlyVeto"]
    ...
    report: dict[str, Any] = {
        "strategy": "Stage25_ConflictOnlyVeto",
        ...
        "feature_policy": {
            ...
            "macro_confidence": (
                "clip(1 + min(sign(macro_mu-neutral_mu) * "
                "technical_direction, 0), 0, 1); agreement and neutral "
                "signals do not shrink the macro view"
            ),
            "filtered_macro_mu": (
            ...
            "single_change_from_stage20": (
                "technical confidence is spent only on direction conflict"
            ),
            "research_status": (
                "exploratory follow-up declared after Stage 21-24 diagnostics"
            ),
        },
        "unchanged_controls": {
            "downside_risk_aversion_lambda": 1.0,
    ...
    if save:
        ...
        technical_path.to_csv(
            OUTPUT_DIR / "conflict_only_veto_monthly.csv"
        )
