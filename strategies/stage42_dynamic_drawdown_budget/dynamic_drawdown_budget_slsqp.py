...
STRATEGY_NAME = "Stage42_DynamicDD10_ExAnteSharpe11"
STAGE36_NAME = "Stage36_Frozen"
STAGE36_MODE = "gvz_ovx_asset_risk"

NAV_FLOOR_RATIO = 0.90
...
class DrawdownFloorAlreadyBreached(RuntimeError):
    """The realized strategy NAV is already below 90% of its running peak."""
...
def remaining_loss_budget(current_drawdown: float) -> float:
    """Return the additional loss that would place NAV exactly at 90% of peak.

    If current NAV is ``(1+d) * peak``, the next-period return at the hard floor
    solves ``(1+d) * (1+L) = 0.90``.  No fitted risk multiplier is involved.
    """
...
def build_stage36_forecast(
    ...
    variance-only scaling.  Stage42 changes the optimization layer, not the
...
def portfolio_forecast_statistics(
    ...
    """Compute the Stage42 objective and both strict constraints."""
...
def solve_weights(
    ...
    if current_drawdown <= MDD_FLOOR - CONSTRAINT_TOLERANCE:
        raise DrawdownFloorAlreadyBreached(
            f"Current drawdown {current_drawdown:.6%} is already below -10%."
        )
    ...
    if not feasible:
        ...
        raise InfeasiblePortfolioError(
            "No feasible portfolio without relaxing Stage42 constraints: "
...
def run_research(save: bool = True) -> dict[str, Any]:
    ...
    report = {
        ...
        "constraints": {
            "ex_ante_sharpe": ">= 1.10",
            "dynamic_tail_risk": (
                "historical CDaR(90%) >= 0.90/(1+current realized DD)-1"
            ),
            "weights": "sum=1 and each in [0,1]",
            "final_validation": "realized MDD >= -10% and Sharpe >= 1.10",
        },
        "preserved": {
    ...
    if save:
        ...
        candidate.to_csv(OUTPUT_DIR / "stage42_dynamic_dd_budget_monthly.csv")
