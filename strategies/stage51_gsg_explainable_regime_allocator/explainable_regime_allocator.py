...
"""Stage50의 USO를 GSG로 교체한 설명 가능한 거시 국면 자산배분 전략.

이 파일은 Stage50의 단일 거시 국면 전략에서 원유 ETF(USO)를 broad commodity
index ETF(GSG)로 바꾼 연구 변형이다. 다른 ``strategies.stageXX`` 모듈을 import하지
않는다. 처음 코드를 보는 사람은 아래의 한 방향 흐름만 따라가면 된다.
...
리팩터링의 경계
----------------
이 구현은 Stage50 구조를 최대한 보존하되 투자 대상만 USO에서 GSG로 바꾼다. 따라서
Stage50의 성과를 보존하지는 않는다. 앞선 감사에서 발견된 통계적 한계는 조용히
고치지 않고 코드 가까이에 남긴다.
    ...
    def _market_cache(self, refresh: bool) -> pd.DataFrame:
        ...
        if cache.exists() and not refresh:
            ...
            if missing:
                raise ValueError(
                    ...
                    "Run Stage51 with --refresh-monthly-market-cache to download GSG."
                )
            return cached
        ...
        try:
            ...
            raise ModuleNotFoundError(
                "Refreshing Stage51 market cache requires yfinance. "
    ...
    def load(self) -> tuple[pd.DataFrame, dict[str, Any]]:
        ...
        if missing:
            raise ValueError(
                ...
                "For GSG, run Stage51 with --refresh-monthly-market-cache first."
            )
...
class BayesianRegimeReturnForecaster:
    ...
    * predictive_std에는 잔차와 coefficient 불확실성이 함께 들어 있다. Stage50처럼
      이를 기록만 하고 배분 공분산에는 넣지 않는다.
    """
...
def performance_table(
    ...
    for name, (start, end) in periods.items():
        ...
        rows.append(
            {
                "Strategy": "Stage51_GSGExplainableRegimeAllocator",
                "Period": name,
