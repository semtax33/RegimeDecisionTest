...
"""Rolling z-score growth/inflation으로 betting size를 정하는 Stage56 전략.

이 파일은 Stage51의 GSG universe와 Bayesian ridge 기대수익 추정은 유지하되,
국면확률 대신 rolling z-score 기반 성장 score, 물가 score, 성장x물가 score만
regression feature로 쓴다.
...
아래의 한 방향 흐름만 따라가면 된다.
    ...
    원자료 -> 월별 자산수익률 / 거시 국면점수 / 일별 KRW 수익률
           -> rolling-z g_score, i_score, g_score * i_score
...
리팩터링의 경계
----------------
이 구현은 Stage55의 GSG universe와 Bayesian z allocator를 보존하되 성장/물가
score 계산을 rolling z-score로 바꾼다. 따라서 Stage50/51의 SLSQP 위험예산 충족
성과를 보존하지 않는다. 앞선
...
class StrategyConfig:
    ...
    # GROWTH/INFLATION EXPERIMENT: 4개 regime score 대신 원천 score와 interaction만
    # 사용한다. 사용자가 요청한 그대로 rank(g*i)는 만들지 않는다.
    forecast_features: tuple[str, ...] = (
        "g_score",
        "i_score",
        "g_i_score",
    )
    rolling_z_clip: float = 3.0
    macro_zscore_windows: tuple[tuple[str, int], ...] = (
        ("GDP_YoY", 72),
        ("Export_YoY", 36),
        ("BSI", 24),
        ("CPI_YoY", 36),
...
class MacroRegimeRepository:
    """여섯 거시지표를 rolling z-score 성장/물가 score로 변환한다.

    통계적 의미
    -----------
    GDP YoY는 72개월, 수출 YoY와 물가지표는 36개월, BSI는 24개월 rolling
    z-score를 사용한다. 각 z-score는 [-3, 3]으로 winsorize한 뒤 성장 3개와
    물가 3개를 각각 동일가중 평균한다. ``g_i_score``는 두 score의 단순 곱이다.
    ...
    def build_scores(
        ...
        zscores = pd.DataFrame(index=levels.index)
        for column, window in self.config.macro_zscore_windows:
            zscores[f"{column}_z"] = self._rolling_zscore(
                levels[column], window, self.config.rolling_z_clip
            )
        growth_columns = ["GDP_YoY_z", "Export_YoY_z", "BSI_z"]
        inflation_columns = ["CPI_YoY_z", "PPI_YoY_z", "ImportPrice_YoY_z"]
        zscores["g_score"] = zscores[growth_columns].mean(axis=1)
        zscores["i_score"] = zscores[inflation_columns].mean(axis=1)
        zscores["g_i_score"] = zscores["g_score"] * zscores["i_score"]
        rows: list[dict[str, Any]] = []
        for decision_month in decision_months:
            signal_month = decision_month - 1
            known_rank = ranks.loc[: signal_month.to_timestamp("M")]
            known_zscore = zscores.loc[: signal_month.to_timestamp("M")]
            if known_rank.empty or known_zscore.empty:
                continue
            if known_rank.iloc[-1].isna().any() or known_zscore.iloc[-1].isna().any():
                continue
            current_rank = known_rank.iloc[-1]
            current_zscore = known_zscore.iloc[-1]
            growth_high = float(current_rank[["GDP_YoY", "Export_YoY", "BSI"]].mean())
            inflation_high = float(
                current_rank[["CPI_YoY", "PPI_YoY", "ImportPrice_YoY"]].mean()
            )
            rows.append(
                {
                    ...
                    **{
                        column: float(current_zscore[column])
        ...
        audit = {
            ...
            "score_method": "rolling_zscore",
            "zscore_windows": dict(self.config.macro_zscore_windows),
            "zscore_clip": self.config.rolling_z_clip,
            "growth_score_columns": growth_columns,
            "inflation_score_columns": inflation_columns,
            "interaction": "g_score * i_score, no rank transform",
            "legacy_regime_probabilities_retained_for_diagnostics": True,
...
class BacktestEngine:
    ...
    def _feature_frame(
        ...
        frame = regime_scores.loc[:, feature_columns].copy()
        for asset in self.config.assets:
    ...
    def run(
        ...
        for month in months:
            ...
            row: dict[str, Any] = {
                ...
                **{
                    column: float(regime_scores.loc[month, column])
                    for column in self.config.forecast_features
                },
                **{
...
def performance_table(
    ...
    for name, view in periods.items():
        rows.append(
            {
                "Strategy": "Stage56_RollingZGrowthInflationBayesianZ",
                "Period": name,
...
def save_weight_artifacts(
    ...
    for period, view in analysis_periods(monthly_result, config).items():
        ...
        ax.set_title(f"Stage56 Portfolio Weights - {period}")
...
def individual_asset_performance_table(
    ...
    for period, view in analysis_periods(monthly_result, config).items():
        rows.append(
            {
                "Period": period,
                "Series": "Stage56_RollingZGrowthInflationBayesianZ",
                "Type": "strategy_net",
                "Start": str(view.index.min()),
                "End": str(view.index.max()),
                **_performance_summary(view["return"]),
...
def save_pnl_comparison_artifacts(
    ...
    for period, view in analysis_periods(monthly_result, config).items():
        ...
        returns["Stage56"] = view["return"].astype(float)
        for asset in config.assets:
        ...
        ax.set_title(f"Stage56 vs Single-Asset Buy-and-Hold PnL - {period}")
...
def run_research(
    *,
    ...
    if save:
        ...
        regime_scores.to_csv(paths.output / "macro_score_inputs.csv")
    ...
    return {
        ...
        "macro_score_inputs": regime_scores,
