...
"""Growth/inflation score로 betting size를 정하는 Stage55 자산배분 전략.

이 파일은 Stage51의 GSG universe와 Bayesian ridge 기대수익 추정은 유지하되,
국면확률 대신 성장 score, 물가 score, 성장x물가 score만 regression feature로 쓴다.
...
아래의 한 방향 흐름만 따라가면 된다.
    ...
    원자료 -> 월별 자산수익률 / 거시 국면점수 / 일별 KRW 수익률
           -> g_score, i_score, g_score * i_score
...
리팩터링의 경계
----------------
이 구현은 Stage52의 GSG universe와 Bayesian z allocator를 보존하되 regression
feature를 세 개의 성장/물가 score로 바꾼다. 따라서 Stage50/51의 SLSQP 위험예산
충족 성과를 보존하지 않는다. 앞선
            ...
            rows.append(
                {
                    ...
                    "g_score": growth_high,
                    "i_score": inflation_high,
                    "g_i_score": growth_high * inflation_high,
...
def performance_table(
    ...
    for name, view in periods.items():
        rows.append(
            {
                "Strategy": "Stage55_GrowthInflationBayesianZ",
                "Period": name,
...
def save_weight_artifacts(
    ...
    for period, view in analysis_periods(monthly_result, config).items():
        ...
        ax.set_title(f"Stage55 Portfolio Weights - {period}")
...
def individual_asset_performance_table(
    ...
    for period, view in analysis_periods(monthly_result, config).items():
        rows.append(
            {
                "Period": period,
                "Series": "Stage55_GrowthInflationBayesianZ",
                "Type": "strategy_net",
                "Start": str(view.index.min()),
                "End": str(view.index.max()),
                **_performance_summary(view["return"]),
...
def save_pnl_comparison_artifacts(
    ...
    for period, view in analysis_periods(monthly_result, config).items():
        ...
        returns["Stage55"] = view["return"].astype(float)
        for asset in config.assets:
        ...
        ax.set_title(f"Stage55 vs Single-Asset Buy-and-Hold PnL - {period}")
