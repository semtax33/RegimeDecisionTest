...
def build_daily_technical_features(
    ...
    for asset in ASSETS:
        ...
        if asset == "KODEX200":
            ...
            # The economically aligned 126-day K-Ratio owns the monthly
            # equity direction.  The two 14-day RSI series remain saved as
            # diagnostics but do not receive directional voting power after
            # their Stage 20 next-month IC was indistinguishable from zero.
...
def solve_weights(
    ...
    detail: dict[str, Any] = {
        **values,
            "policy": "KRatioEquityVeto_StaticLambda",
...
def run_research(save: bool = True) -> dict[str, Any]:
    ...
    paths = {
        ...
        "Stage27_KRatioEquityVeto": technical_path,
    }
    ...
    episodes = pd.concat(
        [
            ...
            drawdown_episodes(
                technical_path, returns, "Stage27_KRatioEquityVeto"
            ),
        ],
        ignore_index=True,
    )
    ...
    candidate = full.loc["Stage27_KRatioEquityVeto"]
    ...
    report: dict[str, Any] = {
        "strategy": "Stage27_KRatioEquityVeto",
        ...
        "feature_policy": {
            ...
            "equity_technical_direction": "k_score",
            "equity_rsi_role": (
                "saved for diagnostics only; no directional vote"
            ),
            "other_asset_technical_direction": "k_score",
            "macro_confidence": (
                "KODEX200: clip(1 + min(agreement,0),0,1) using KScore; "
            ...
            "candidate_count": 1,
            "combined_changes_from_stage20": (
                "K-Ratio is the sole equity direction input and KODEX200 "
                "uses conflict-only confidence"
            ),
            "research_status": (
                "exploratory combination declared after Stage24 and Stage26"
            ),
        },
        "unchanged_controls": {
            "downside_risk_aversion_lambda": 1.0,
    ...
    if save:
        ...
        technical_path.to_csv(
            OUTPUT_DIR / "k_ratio_equity_veto_monthly.csv"
        )
