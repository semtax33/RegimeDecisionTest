...
def apply_technical_inputs(
    ...
    absolute_scale = 1.0 + np.clip(atr_percentile, 0.0, 1.0)
    # ATR now ranks current risk across the investable assets without
    # changing the optimizer's aggregate variance-aversion budget.  The
    # arithmetic normalization is parameter-free and has mean exactly one.
    variance_scale = absolute_scale / float(absolute_scale.mean())
    ...
    return {
        ...
        "atr_absolute_scale": absolute_scale,
...
def solve_weights(
    ...
    detail: dict[str, Any] = {
        **values,
            "policy": "RelativeATR_StaticLambda",
...
def run_backtest(
    ...
    for month in months:
        ...
        vector_fields = {
            ...
            "atr_percentile": "atr_percentile",
            "atr_absolute_scale": "atr_absolute_scale",
...
def run_research(save: bool = True) -> dict[str, Any]:
    ...
    paths = {
        ...
        "Stage23_RelativeATR": technical_path,
    }
    ...
    episodes = pd.concat(
        [
            ...
            drawdown_episodes(
                technical_path, returns, "Stage23_RelativeATR"
            ),
        ],
        ignore_index=True,
    )
    ...
    candidate = full.loc["Stage23_RelativeATR"]
    ...
    report: dict[str, Any] = {
        ...
        "feature_policy": {
            ...
            "atr_covariance": (
                "raw_i=1+causal_ATR_percentile_i; scale_i=raw_i/mean(raw); "
                "D @ Sigma @ D where D_i=sqrt(scale_i)"
            ),
            "searched_parameters": None,
            "candidate_count": 1,
            "single_change_from_stage20": (
                "cross-sectionally normalized relative ATR covariance scale"
            ),
        },
        "unchanged_controls": {
            "downside_risk_aversion_lambda": 1.0,
    ...
    if save:
        ...
        technical_path.to_csv(
            OUTPUT_DIR / "relative_atr_monthly.csv"
        )
